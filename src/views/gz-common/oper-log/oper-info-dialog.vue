<template>
  <el-dialog v-model="open" title="谷子操作日志详情" width="700px" append-to-body close-on-click-modal @closed="info = null">
    <el-descriptions v-if="info" :column="1" border>
      ...
      <el-descriptions-item label="消耗时间">
        <template #default> <span> {{ info.costTime }}ms </span> </template>
      </el-descriptions-item>
...
import type { GzOperLogVO } from '@/api/gz-common/oper-log';
import VueJsonPretty from 'vue-json-pretty';
import 'vue-json-pretty/lib/styles.css';

const open = ref(false);
const info = ref<GzOperLogVO | null>(null);
function openDialog(row: GzOperLogVO) {
  info.value = row;
  open.value = true;
}
...
const typeFormat = (row: GzOperLogVO) => {
